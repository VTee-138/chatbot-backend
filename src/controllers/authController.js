...
const register = catchAsync(async (req, res, next) => {
  try {
    ...
    if (pendingVerification) {
      return errorResponse(
        res,
        "Vui lòng kiểm tra mail xác nhận để kích hoạt tài khoản!",
        Constants.BAD_REQUEST
      );
    }
    ...
    await redis.set(
      ...
      validateToken,
      "EX",
      Constants.TIME_PICKER._120secs
    );
    ...
    httpOnlyResponse(
      ...
      "registerEmail",
      email,
      Constants.TIME_PICKER._1hour_ms
    );
    httpOnlyResponse(
      res,
      "registerEmailToken",
    ...
    try {
      sendEmailToVerify(
        ...
        validateToken,
        email,
        "🚀 Link xác thực tài khoản đăng ký đã tới!",
        HtmlConverter.Register
      );
...
const login = catchAsync(async (req, res, next) => {
  ...
  try {
    ...
    if (!user || !user.emailVerifiedAt || !user.passwordHash) {
      return errorResponse(
        res,
        "Email hoặc mật khẩu không hợp l",
        Constants.BAD_REQUEST
      );
    }
    // Verify password
...
const refreshToken = catchAsync(async (req, res) => {
  ...
  if (!cookieClientInformation) {
    ...
    return errorResponse(
      res,
      "Client information not found. Please login again.",
      401
    );
  }
  ...
  try {
    ...
    if (
      !cookieUserInformation.id ||
      !cookieUserInformation.email ||
      !cookieUserInformation.userName
    ) {
    ...
    logger.debug(
      `Checking refresh token for user: ${cookieUserInformation.userName} (${cookieUserInformation.id})`
    );

    // Check Redis with better error handling
    let storedHashedToken;
    try {
      storedHashedToken = await redis.get(
        `refresh:${cookieUserInformation.id}`
      );
    } catch (redisError) {
    ...
    if (!storedHashedToken) {
      logger.error(
        " No refresh token found in Redis for user:",
        cookieUserInformation.id
      );
    ...
    if (!isValidRefreshToken) {
      logger.error(
        " Refresh token verification failed for user:",
        cookieUserInformation.id
      );
      ...
      try {
        ...
        logger.warn(
          "⚠️ Failed to delete refresh token from Redis:",
          redisError.message
        );
    ...
    await redis.set(
      `refresh:${user.id}`,
      newHashedToken.valueOf(),
      "EX",
      Constants.TIME_PICKER._7day_secs
    );
    ...
    httpOnlyResponse(
      ...
      "refreshToken",
      tokens.refreshToken,
      Constants.TIME_PICKER._7day_ms
    );
    httpOnlyResponse(
      res,
      "clientInformation",
  ...
  try {
    ...
    try {
      ...
      sendEmailToVerify(
        ...
        token,
        email,
        "🚀 Link xác nhận quên mật khẩu đã tới!",
        HtmlConverter.Forgot
      );
...
const logout = catchAsync(async (req, res) => {
  ...
  try {
    ...
    // 1. Try from JWT token (if authenticated via middleware)
    if (req.user && req.user.id) {
      id = req.user.id;
      console.log(`📋 Got user ID from JWT: ${id}`);
    } 
    // 2. Try from cookies (manual parse)
    else if (req.cookies.clientInformation) {
      try {
        const clientInfo = JSON.parse(req.cookies.clientInformation);
        id = clientInfo.id;
        console.log(`📋 Got user ID from cookies: ${id}`);
      } catch (parseError) {
        console.log("⚠️ Failed to parse clientInformation cookie");
      }
    }

    // Clean up Redis if we have user ID
    if (id) {
      ...
      try {
        ...
        console.log("⚠️ Redis cleanup failed:", redisError.message);
      }

      // Clear rate limiting (non-critical)
      try {
        rateLimiterAuth.delete(req.ip);
        rateLimiterGeneral.delete(req.ip);
      } catch (limiterError) {
        console.log("⚠️ Rate limiter cleanup failed:", limiterError.message);
      }
    } else {
      console.log("⚠️ No user ID found, only clearing cookies");
    }

    // ALWAYS clear cookies (critical part)
    httpOnlyRevoke(res, "refreshToken");
    httpOnlyRevoke(res, "clientInformation");
    httpOnlyRevoke(res, "activeGroup");
    httpOnlyRevoke(res, "forgotEmail");
    httpOnlyRevoke(res, "forgotEmailToken");
    httpOnlyRevoke(res, "registerEmail");
    httpOnlyRevoke(res, "registerEmailToken");
    ...
    // Still return success (cookies cleared)
...
const getProfile = catchAsync(async (req, res) => {
  ...
  console.log(
    `🔗 User has SSO providers: ${ssoProviders
      .map((s) => s.provider)
      .join(", ")}`
  );
...
const changePassword = catchAsync(async (req, res) => {
  ...
  const user = await userCredentialModel.findUserById(userId);

  if (!user) {
    return errorResponse(res, "User not found", Constants.NOT_FOUND);
  }
  ...
  if (!isCurrentPasswordValid) {
    return errorResponse(res, "Incorrect current password", Constants.BAD_REQUEST);
  }
  ...
  await userCredentialModel.updatePasswordByID(userId, hashedNewPassword);
...
const resendVerifyEmail = catchAsync(async (req, res, next) => {
  try {
    ...
    // Get email from appropriate cookie based on type
    const email = type === "register" 
      ? req.cookies.registerEmail 
      : req.cookies.forgotEmail;
    ...
    // Get existing token from Redis (instead of cookie)
    ...
    console.log(`♻️ Resending ${type} verification email to: ${email}`);

    // Verify the stored token is still valid
    let decodedInformation;
    try {
      const { iat, exp, ...decoded } = verifyToken(storedToken, "validate");
      decodedInformation = decoded;
    } catch (tokenError) {
      // Token expired or invalid, generate new one
      console.log(`⚠️ Stored token invalid, generating new token`);
    ...
    const emailType =
      type === "forgot" ? EmailType.RESET_PASSWORD : EmailType.REGISTER;
    
    const htmlConverter =
      type === "forgot" ? HtmlConverter.Forgot : HtmlConverter.Register;
    ...
    try {
      ...
      console.log(`✅ Resent ${type} verification email successfully`);
      ...
      // Token đã được tạo và lưu, chỉ email service lỗi
      return successResponse(
        res,
        "Yêu cầu đã được xử lý. Nếu không nhận được email, vui lòng thử lại sau.",
        200
    ...
    if (error.name === "JsonWebTokenError") {
      return errorResponse(
        res,
        "Invalid session. Please restart the process",
        Constants.BAD_REQUEST
      );
    }
    next(error);
...
const resetPassword = catchAsync(async (req, res, next) => {
  try {
    ...
    const hashedNewPassword = await hashPassword(newPassword);
    await userCredentialModel.updatePassword(
      email,
      hashedNewPassword.valueOf()
    );
...
const openSession = catchAsync(async (req, res, next) => {
  try {
    ...
    httpOnlyResponse(
      ...
      "accessToken",
      tokens.accessToken,
      Constants.TIME_PICKER._15mins_ms
    );
    httpOnlyResponse(
      res,
      "refreshToken",
...
const googleOAuthCallback = catchAsync(async (req, res, next) => {
  ...
  try {
    ...
    if (error) {
      ...
      return res.redirect(
        `${frontendUrl}/login?error=oauth_error&provider=google`
      );
    }
    ...
    try {
      ...
      const frontendUrl = config.FRONTEND_URL || "http://localhost:3000";
      return res.redirect(
        `${frontendUrl}/login?error=profile_verification_failed`
      );
    }
    ...
    let ssoUser = await userCredentialModel.findUserBySSO(
      "google",
      ssoProfile.sub
    );
    ...
    if (!ssoUser) {
      ...
      let userName = convertToAscii(
        ssoProfile.userName ||
          ssoProfile.name ||
          `user_${ssoProfile.sub.slice(-8)}`
      );
      ...
      try {
        ssoUser = await userCredentialModel.createSSOAccount(
          "google",
          ssoProfile.sub,
          {
        ...
        const frontendUrl = config.FRONTEND_URL || "http://localhost:3000";
        return res.redirect(
          `${frontendUrl}/login?error=account_creation_failed`
        );
      }
    } else {
    ...
    if (ssoUser.twoFactorEnabled) {
      ...
      const frontendUrl = config.FRONTEND_URL || "http://localhost:3000";
      return res.redirect(
        `${frontendUrl}/verify-2fa?token=${mfaToken}&provider=google`
      );
    }
...
const loginSSO = catchAsync(async (req, res, next) => {
  ...
  try {
    ...
    if (!provider || !["google", "facebook"].includes(provider)) {
      return errorResponse(
        res,
        "Phương thức đăng nhập không được hỗ trợ. Vui lòng sử dụng Google hoặc Facebook",
        Constants.BAD_REQUEST
      );
    }
    ...
    try {
      ...
      return errorResponse(
        res,
        `Invalid ${provider} token or authentication failed`,
        Constants.UNAUTHORIZED
      );
    }
    ...
    if (!ssoProfile || !ssoProfile.sub) {
      return errorResponse(
        res,
        `Failed to get ${provider} profile information`,
        Constants.BAD_REQUEST
      );
    }

    console.log(
      `✅ ${provider} profile verified for user: ${
        ssoProfile.email || ssoProfile.sub
      }`
    );
    ...
    if (!ssoUser) {
      ...
      try {
        ...
        console.log(
          `✅ Created new ${provider} account for user: ${newUserName}`
        );
      } catch (createError) {
        console.error("Failed to create SSO account:", createError);
        return errorResponse(
...
const checkSession = catchAsync(async (req, res) => {
  ...
  if (req.user && req.user.id) {
    console.log(
      "✅ Session verified via JWT token for user:",
      req.user.userName
    );
  ...
  console.log(
    "📋 Client info from cookies:",
    clientInfo ? "EXISTS" : "MISSING"
  );
  ...
  // Check fields
  if (!refreshHash)
    return errorResponse(res, "Session not available", Constants.BAD_REQUEST);
  const checker = await comparePassword(token, refreshHash);
  console.log("✅ Session verification:", checker ? "VALID" : "INVALID");

  // Return
  if (checker)
    return successResponse(res, { sessionValid: true }, "Session is active");
  else
    return errorResponse(res, "Session not available", Constants.BAD_REQUEST);
});
...
const twoFactorVerify = catchAsync(async (req, res, next) => {
  try {
    ...
    if (!checker)
      return errorResponse(
        res,
        Constants.MESSAGES._TOKEN_INVALID,
        Constants.BAD_REQUEST
      );
...
const twoFactorBackupCodeVerify = catchAsync(async (req, res, next) => {
  try {
    ...
    const isValid = await TwoFAService.verifyBackupCode(
      token,
    ...
    const newBackupCodes = await TwoFAService.removeUsedBackupCode(
      token,
      user.twoFactorBackupCodes
    );
  ...
  try {
    ...
    if (ssoProviders && ssoProviders.length > 0) {
      ...
      if (!idToken || !provider) {
        return errorResponse(
          res,
          "SSO provider and idToken are required",
          Constants.BAD_REQUEST
        );
      }

      const ssoAccountsInDB = await userCredentialModel.findSSOUserById(
        currentUserId
      );
      const ssoProfile =
        provider === "google"
      ...
      for (const ssoAccount of ssoAccountsInDB) {
        if (
          ...
          ssoAccount.providerId === ssoProfile.sub
        ) {
      ...
      const currentUser = await userCredentialModel.findAccountWithUserName(
        currentUserName
      );
      if (
        currentUser &&
        (await comparePassword(password, currentUser.passwordHash))
      ) {
        isReAuthenticated = true;
      }
    }
    ...
    if (isReAuthenticated) {
      ...
      return errorResponse(
        res,
        "Re-authentication failed. Invalid credentials.",
        Constants.UNAUTHORIZED
      );
    }
  } catch (error) {
...
const debugCookies = catchAsync(async (req, res) => {
  ...
  if (!req.cookies.refreshToken) {
    debugInfo.recommendations.push(
      "No refresh token cookie found. User needs to login."
    );
  }

  if (!req.headers.cookie) {
    debugInfo.recommendations.push(
      'No Cookie header in request. Check if frontend is sending credentials: "include"'
    );
  }

  if (
    req.cookies.refreshToken &&
    req.cookies.clientInformation &&
    !debugInfo.redis.exists
  ) {
    debugInfo.recommendations.push(
      "Cookies exist but no Redis session. Session may have expired."
    );
  }
